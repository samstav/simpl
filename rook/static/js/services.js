...
services.factory('auth', ['$http', '$resource', '$rootScope', '$q', function($http, $resource, $rootScope, $q) {
  ...
  // ============================== Wrapper for $http ==============================
  var createShortMethods = function(names) {
    angular.forEach(arguments, function(name) {
      $http[name] = function(url, config) {
        var original_call = function() {
          return $http(angular.extend(config || {}, {
            method: name,
            url: url
          }));
        }
        return impersonation_wrapper(original_call);
      };
    });
  }

  var createShortMethodsWithData = function(names) {
    angular.forEach(arguments, function(name) {
  ...
  var impersonation_wrapper = function( callback, config ) {
    var result;

    // TODO: check to see if it's a tenant call (tenant ID in URL)
    if (auth.is_admin(true) && !auth.impersonation_in_progress) {
      // Wrapped Call
      var temporarily = true;
      var username = 'rackcloudtech'; // TODO: get username from tenant ID

      result = auth.impersonate(username, temporarily).then(
        // Success
        function(response) {
          var result = callback();
  ...
  // If you want to wrap $http calls with impersonation
  // just uncomment the following line(s):
