...
from openstack.compute import exceptions as openstack_exceptions
...
class Provider(provider.RackspaceComputeProviderBase):
    ...
    def generate_template(self, deployment, resource_type, service, context,
        ...
        if not matches:
            raise cmexc.CheckmateNoMapping("No flavor has at least "
                                           "'%s'memory" % memory)
    ...
    def add_resource_tasks(self, resource, key, wfspec, deployment, context,
                           wait_on=None):
        """Add all tasks for the given resource
    ...
    def get_catalog(self, context, type_filter=None, **kwargs):
        """Return catalog: stored/override if available, else build one."""
        # TODO(any): maybe implement this an on_get_catalog so we don't have to
        #       do this for every provider
        results = provider.RackspaceComputeProviderBase.get_catalog(
            self,
            context,
        ...
        if type_filter is None or type_filter == 'compute':
            ...
            win_opts = results['compute']['windows_instance']['options']
            if not images:
            ...
            win_opts['os']['display-hints']['choice'] = win_choices
            win_opts['os']['constraints'].append({'in': win_allowed})
            ...
            win_opts['memory']['display-hints']['choice'] = win_choices
    ...
    @staticmethod
    def _images(api):
        """Gets current tenant's images and formats them for Checkmate."""
    ...
    @staticmethod
    def _flavors(api):
        """Return current tenant's flavors in Checkmate's format."""
...
def create_server(context, name, api_object=None, flavor=2, files=None,
    ...
    try:
        ...
        raise
    except openstack_exceptions.OverLimit as exc:
        raise cmexc.CheckmateException(
            str(exc),
    ...
    tasks.resource_postback.delay(context['deployment'], results)
    return results


@task.task(default_retry_delay=30, max_retries=120)
@statsd.collect
def wait_on_build(context, server_id, ip_address_type='public',
    ...
    if not server_ip:
        ...
        up = ssh.test_connection(context, server_ip, username, timeout=timeout,
                                 password=password,
