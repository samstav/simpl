...
class Provider(RackspaceComputeProviderBase):
    ...
    def generate_template(self, deployment, resource_type, service, context,
                          name=None):
        template = RackspaceComputeProviderBase. \
        ...
        flavor = deployment.get_setting('memory', resource_type=resource_type,
                                        service_name=service,
                                        provider_key=self.key, default=2)
...
def wait_on_build(context, id, ip_address_type='public',
    ...
    if not ip:
        ...
        if up:
            ...
            initialize_monitoring.delay(ip=ip,name="Entity for %s" %
                id,context=context['deployment'], resource="node")
            # Send data back to deployment
            resource_postback.delay(context['deployment'], results)
            return results
