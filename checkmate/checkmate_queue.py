...
#        params.extend(['celeryd', '--config=checkmate.celeryconfig', '-I',
        ...
        params.extend(['-m','celery.bin.celeryd', '--config=checkmate.celeryconfig', '-I',
...
#        params = [sys.executable, 'celeryev', '--config=checkmate.celeryconfig']
        params = [sys.executable, '-m', 'celery.bin.celeryev', '--config=checkmate.celeryconfig']
        try:
            print 'Running: %s' % ' '.join(params)
            call(params)
        except KeyboardInterrupt:
